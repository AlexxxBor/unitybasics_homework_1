...
namespace Sample
{
    public static class LessonTypes
    {
        ...
        {
            return (deg + min / 60.0 + sec / 60.0 / 60.0) * (Math.PI / 180);
        }
        ...
        {
            int timeDepart = TimeToMinutes(hoursDepart, minutesDepart);
            int timeArrive = TimeToMinutes(hoursArrive, minutesArrive);
            return timeArrive - timeDepart;

            int TimeToMinutes(int h, int m = 0) => h * 60 + m;
        }
         ...
         * Например, 100 рублей под 10% годовых превратятся в 133.1 рубля !за 3 года!
         */
        public static double CalcMoney(int initialMoney, int percent, int years) => initialMoney * Math.Pow(1 + percent / 100.0, years);
        ...
        {
            int newHundreds = number % 10 * 100;
            int newTens = number / 10 % 10 * 10;
            int newUnits = number / 100;

            return newHundreds + newTens + newUnits;
        }
    }
}